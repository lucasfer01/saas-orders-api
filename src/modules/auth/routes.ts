...
import { conflict, notFound, badRequest } from "../../http/errors.js";
...
export const authRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/auth/register", async (req, reply) => {
    ...
    const refreshRow = await app.prisma.refreshToken.create({
      data: {
        ...
        expiresAt: new Date(Date.now() + Number(process.env.JWT_REFRESH_TTL_DAYS ?? 14) * 86400_000),
      },
  ...
  app.post("/auth/refresh", async (req) => {
    ...
    if (row.expiresAt.getTime() < Date.now()) throw badRequest("Refresh token expired");
